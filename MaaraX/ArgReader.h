...
namespace NVL_Utils 
{
    ...
    public:
        ...
        inline static NVLib::Parameters * GetParameters(int argc, char ** argv) 
        {
            ...
            parameters->Add("start_index", parser.get<String>("start_index"));
            parameters->Add("element_count", parser.get<String>("element_count"));
    ...
    private:
        ...
        inline static string GetParamKeys() 
        {
            const char * keys = 
                ...
                "{ start_index    | 2                     | The index that we are starting with          }"
                "{ element_count  | 2                     | The location of the output folder            }"; 
